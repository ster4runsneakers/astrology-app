...
def build_chart_figure(chart: NatalChart) -> Figure:
    ...
    for i, p in enumerate(bodies):
        ...
        color = _GOLD if p.id in ("sun", "asc") else (_ACCENT if p.id == "moon" else _TEXT)
        ax.plot(th, r, "o", color=color, markersize=6, zorder=5)
        ax.text(
            th,
            r + 0.08,
            glyph,
            ...
            va="center",
            color=color,
            fontsize=10 if p.id != "asc" else 8,
            fontweight="bold",
            zorder=6,
        )

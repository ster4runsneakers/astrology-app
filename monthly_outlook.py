...
def build_monthly_outlook(
    ...
    for i, (y, m) in enumerate(_next_months(start, months)):
        ...
        text = (
            f"{opener} **{z.name_el}**. {flavor} ο **{MONTHS_EL[m]} {y}** "
            f"({season}) φέρνει φως στο θέμα **{theme}**.\n\n"
            f"Εδώ {middle}. Δώσε χώρο σε {focus}. "
            f"Μην συγκρίνεις αυτόν τον μήνα με τον προηγούμενο — έχει δικό του ρυθμό.\n\n"
            f"{closer}{cn_bit}"
        )

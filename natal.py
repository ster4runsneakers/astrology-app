...
class BirthProfile:
    ...
    birth_time: str = "12:00"  # HH:MM local wall clock in `timezone`
    time_unknown: bool = False
    place_name: str = "Αθήνα"
    latitude: float = 37.9838
    longitude: float = 23.7275
    timezone: str = "Europe/Athens"  # IANA TZ for correct Ascendant
...
class NatalChart:
    ...
    midheaven: Optional[PlanetPosition]
...
def _parse_profile_datetime(profile: BirthProfile) -> datetime:
    """Interpret birth clock as local wall time in profile.timezone → UTC."""
    ...
    try:
        ...
        # Fallback: crude longitude mean-time (legacy)
    ...
    local_dt = datetime(y, m, d, hour, minute, 0, tzinfo=tz)
...
def _sidereal_degrees(t) -> float:
    # Apparent sidereal time (GAST) in degrees — slightly better for Asc than GMST.
    return normalize_longitude(t.gast * 15.0)


def _calculate_ascendant(ramc_deg: float, latitude_deg: float, eps_deg: float) -> float:
    """Tropical Ascendant from RAMC (local apparent sidereal time in degrees).

    Standard Meeus/atan2 form — the previous signs were flipped on both axes,
    which returned the Descendant (exactly 180° wrong: e.g. Cancer instead of Capricorn).
    """
...
def _calculate_midheaven(ramc_deg: float, eps_deg: float) -> float:
    """Ecliptic longitude of the MC from RAMC."""
    ramc = math.radians(ramc_deg)
    eps = math.radians(eps_deg)
    y = math.sin(ramc) * math.cos(eps)
    x = math.cos(ramc)
    return normalize_longitude(math.degrees(math.atan2(y, x)))
...
def compute_natal_chart(profile: BirthProfile) -> NatalChart:
    ...
    if not approximate:
        gast = _sidereal_degrees(t)
        ramc = normalize_longitude(gast + profile.longitude)  # Local Apparent Sidereal Time
        eps = _obliquity_degrees(t.tt)
        asc_lon = _calculate_ascendant(ramc, profile.latitude, eps)
        mc_lon = _calculate_midheaven(ramc, eps)
        ascendant = _to_planet_position("asc", "Ωροσκόπος", "Ascendant", asc_lon)
        midheaven = _to_planet_position("mc", "Μεσουράνημα (MC)", "Midheaven", mc_lon)
    ...
    notes.append(
        f"Εφήμεριδα: Skyfield + JPL DE421 · τροπικό ζώδιο · Equal House · "
        f"τοπική ώρα σε timezone `{profile.timezone or 'UTC'}`."
    )
